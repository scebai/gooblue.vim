...
function! s:create_context() abort
  let d = s:create_colors(
        \ gooblue#palette#dark#create())
  let l = s:create_colors(
        \ gooblue#palette#light#create())
...
call pgmnt#compile(s:create_context(), {
      \   'output': '../colors/gooblue.vim',
      \   'template': './template.vim',
      \ })

function! s:create_xline_context() abort
  let ps = {
        \   'dark': gooblue#palette#dark#create(),
        \   'light': gooblue#palette#light#create(),
        \ }
  let context = {}
...
call pgmnt#compile(
      ...
      \   'output': '../autoload/airline/themes/gooblue.vim',
      \   'template': './template-airline.vim',
      \ })
call pgmnt#compile(
      \ s:create_xline_context(), {
      \   'output': '../autoload/lightline/colorscheme/gooblue.vim',
